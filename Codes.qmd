---
title: Codes
bibliography: Resources/biblio_web.bib
format:
  html:
    page-layout: full
---

A list of the codes developped in the UFR that are open-source.

### gcrack – Solver for Linear Elastic Fracture Mechanics

[![License](https://img.shields.io/badge/License-Apache_2.0-blue.svg)](https://opensource.org/licenses/Apache-2.0)
[![GitHub](https://img.shields.io/badge/GitHub-gcrack-blue)](https://github.com/floiseau/gcrack)
[![PyPI](https://img.shields.io/badge/PyPI-gcrack-green)](https://pypi.org/project/gcrack/)
[![Documentation](https://img.shields.io/badge/Docs-gcrack-blueviolet)](https://floiseau.github.io/gcrack)
[![Zenodo](https://zenodo.org/badge/doi/10.5281/zenodo.18247511.svg)](https://doi.org/10.5281/zenodo.18247511)

`gcrack` is a finite element solver for **linear elastic fracture mechanics**. It is built on [`fenicsx`](https://fenicsproject.org/) and features:

- Incremental crack propagation with indirect load control, ensuring each load step occurs precisely at the propagation threshold.
- Support for arbitrary anisotropic fracture properties, i.e., $G_c(\varphi)$.
- Basic support for heterogeneous media.
- A modular architecture for easy extension.

---

### fragma – Phase-Field Fracture Solver

[![License](https://img.shields.io/badge/License-Apache_2.0-blue.svg)](https://opensource.org/licenses/Apache-2.0)
[![GitHub](https://img.shields.io/badge/GitHub-fragma-blue)](https://github.com/floiseau/fragma)
[![PyPI](https://img.shields.io/badge/PyPI-fragma-green)](https://pypi.org/project/fragma/)
[![Documentation](https://img.shields.io/badge/Docs-fragma-blueviolet)](https://floiseau.github.io/fragma)
[![Zenodo](https://zenodo.org/badge/doi/10.5281/zenodo.15654994.svg)](https://doi.org/10.5281/zenodo.15654994)

`fragma` is a **phase-field fracture solver** based on [`fenicsx`](https://fenicsproject.org/). Key features include:

- Indirect load control via path-following methods.
- Crack propagation in anisotropic media (work in progress).

---

### NeuROM - A Reduced-Order modelling solver 
[![PyPI Downloads](https://img.shields.io/pypi/dm/NeuROM-Py.svg?label=PyPI%20downloads)](
https://pypi.org/project/NeuROM-Py/)
[![GitHub license](https://img.shields.io/github/license/alexandredabyseesaram/neurom.svg)](https://github.com/alexandredabyseesaram/neurom)
[![DOI](https://zenodo.org/badge/DOI/10.5281/zenodo.13772741.svg)](https://doi.org/10.5281/zenodo.13772741)

:::: {.columns}

::: {.column width="25%"}

![](Resources/Images/NeuROM_ter.svg){width="100%" fig-align="center"}

:::

::: {.column width="75%"}

:::: {style='margin-top: 2.5em;'}
::::

The reduced-order modelling code used to create surrogate models based on an hybridisation of standard reduced-order modelling methods such as the PGD and Deep learning methods is [public](https://github.com/AlexandreDabySeesaram/NeuROM) and [notebooks with tutorials are also available](https://fenni-pgd-aldabyse-d62c0089e00d70057d8fae6bbaedf6cf1d181c44edf5.gitlabpages.inria.fr/).


:::

::::


